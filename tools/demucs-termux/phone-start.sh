...
# ស្គ្រីបនេះពិនិត្យ **ជំនាន់ចុងក្រោយ** (1.1) ដូច្នេះការរត់វាម្តងទៀតក៏ធ្វើបច្ចុប្បន្នភាព
# ឯកសារ API ឲ្យស្វ័យប្រវត្តិ — មិនត្រូវចងចាំតើជំនាន់ណាកំពុងរត់ទេ។
api_current() { curl -s --max-time 5 "http://127.0.0.1:$PORT/" 2>/dev/null | grep -q '"version":"1.1"'; }
...
if api_up; then
  if api_current; then
    ...
    # API ជំនាន់ចាស់ (គ្មាន version/model ក្នុងចម្លើយ `/` ឬជំនាន់មុន 1.1 ដែលចាប់
    # python/torch ថ្មីរាល់ request) នឹងធ្វើឲ្យគេហទំព័របរាជ័យ ឬយឺតខ្លាំង។ ដូច្នេះយើងបិទវា
    # រួចបើកជំនាន់ថ្មីជំនួស។
    # (tunnel មិនប៉ះពាល់ទេ ដូច្នេះ URL ដដែលនៅបន្តប្រើបាន។)
    echo "API កំពុងរត់ តែជាជំនាន់ចាស់ — កំពុងធ្វើបច្ចុប្បន្នភាពទៅ 1.1..."
